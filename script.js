...
menu.addEventListener("click", function (event) {
    ...
    if (parentButton) {
        ...
        const pryce = parseFloat(parentButton.getAttribute("data-pryce"))
        addToCart(name, pryce)
    }
...
function addToCart(name, pryce) {
    const existingItem = cart.find(item => item.name === name)

    if (existingItem) {
        //se o item já existe, aumenta apena a qtd +1
    ...
    cart.forEach(item => {
        ...
        <div class="flex items-center justify-between">
          <div>
            ...
            <p class=""font-medium mt-2>R$ ${item.pryce.toFixed(2)}</p>
          </div>
     ...
     `
        ...
        total += item.pryce * item.quantity;
...
checkoutBtn.addEventListener("click", function () {
    ...
    const cartItems = cart.map((item) => {
        return (
            ` ${item.name} Quantidade: (${item.quantity}) preço: R$${item.pryce} //`
        )
    }).join("")

    const message = encodeURIComponent(cartItems)
    const phone = "18988278141"
